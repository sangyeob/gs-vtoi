...
DEFAULT_ELASTICBOX_URL = os.getenv('ELASTICBOX_URL', 'https://elasticbox.com')
...
def main():
    ...
    try:
        ...
        print >> sys.stderr, ebcli.TermColors.FAIL + auth.message + ebcli.TermColors.ENDC
        sys.exit(1)
    except KeyboardInterrupt:
        sys.exit(1)
    except ebcli.ApiException, api:
        logging.exception(api)
        print >> sys.stderr, ebcli.TermColors.FAIL + api.message + ebcli.TermColors.ENDC

        sys.exit(2)
    except Exception, ex:
        logging.exception(ex)
        print >> sys.stderr, ebcli.TermColors.FAIL + ex.message + ebcli.TermColors.ENDC
